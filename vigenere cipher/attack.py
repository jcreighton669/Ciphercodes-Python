...
def _decypher(cyphertext, key):
    ...
    decyphered_blocks = processing.to_blocks(
        [fa.shift(col, shift) for col, shift in zip(cols, shifts)])
...
def attack(file, method):
    with open(file) as f:
        ...
        if method == 'kasiski':
            ...
            key_len = kasiski.find_key_length(
                cyphertext=cyphertext[0], seq_len=SEQ_LEN, max_key_len=MAX_KEY_LEN)
        elif method == 'ic':
            print('Applying index of coincidence examination\n')
            key_len = ic.find_key_length(
                cyphertext=cyphertext[0], max_key_len=MAX_KEY_LEN)
...
attack('Network Security\HW1-Beta Test\Beta-Test-Ciphertext.txt', 'kasiski')
attack('Network Security\HW1-Beta Test\Beta-Test-Ciphertext.txt', 'ic')
