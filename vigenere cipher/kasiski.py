...
def _candidate_key_lengths(factor_lists, max_key_len):
    all_factors = [factor_lists[lst][fac] for lst in range(
        len(factor_lists)) for fac in range(len(factor_lists[lst]))]
    ...
    sorted_candidates = sorted(
        set(candidate_lengths), key=lambda x: all_factors.count(x), reverse=True)
...
def find_key_length(cyphertext, seq_len, max_key_len):
    ...
    ckl = _candidate_key_lengths(
        factor_lists=factor_lists, max_key_len=max_key_len)
    key_len = ckl[0]
    return key_len
